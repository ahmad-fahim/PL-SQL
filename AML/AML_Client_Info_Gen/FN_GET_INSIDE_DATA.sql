...
BEGIN
   SELECT SUBSTR (
             INDCLIENTS_ROW_DATA,
               INSTR (INDCLIENTS_ROW_DATA, upper(P_FIRST_COLUMN))
             + LENGTH (upper(P_FIRST_COLUMN)),
               DECODE (INSTR (INDCLIENTS_ROW_DATA, upper(P_LAST_COLUMN)),
                       0, LENGTH (INDCLIENTS_ROW_DATA) + 1,
                       INSTR (INDCLIENTS_ROW_DATA, upper(P_LAST_COLUMN)))
             - (  INSTR (INDCLIENTS_ROW_DATA, upper(P_FIRST_COLUMN))
                + LENGTH (upper(P_FIRST_COLUMN))))
     INTO V_RETURN_DATA
     FROM (SELECT upper(P_ROW_DATA)
                     INDCLIENTS_ROW_DATA
             FROM DUAL);
