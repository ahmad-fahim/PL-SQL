...
IS
   PROCEDURE SP_LOANACCRPOST (V_ENTITY_NUM IN NUMBER, P_CALL_FROM CHAR);
...
IS
   ...
   PROCEDURE INSERT_INTO_LOANIADTL
   IS
   BEGIN
      INSERT INTO LOANIADTL LL (LOANIADTL_ENTITY_NUM,
         ...
         (SELECT V_GLOB_ENTITY_NUM,
           ...
           WHERE     L.RTMPLNIADTL_RUN_NUMBER = W_RUN_NUMBER
                 AND L.RTMPLNIADTL_ACNT_NUM =
                        M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM
                 AND L.RTMPLNIADTL_VALUE_DATE =
   ...
   EXCEPTION
      ...
      THEN
         W_ERR_MSG := SQLERRM;
         W_ERR_MSG :=
               'Unique Key Constraint Violated for A/C '
            || facno (
                  V_GLOB_ENTITY_NUM,
                  M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM)
            || ' at LOANIAMRR/LOANIAMRRDTL on Value Date '
            || M_RTMPLNIA_RECORD (W_ARRAY_PROCESS_SL).RTMPLNIA_VALUE_DATE;
         RAISE E_USEREXCEP;
   END INSERT_INTO_LOANIADTL;

   PROCEDURE INSERT_INTO_LOANIAMRRDTL
   IS
   BEGIN
   ...
   */
   ...
   BEGIN
        SELECT RTMPLNIA_RUN_NUMBER,
          ...
          FROM RTMPLNIA R, ACNTS, LOANACNTS
         WHERE     R.RTMPLNIA_RUN_NUMBER = W_RUN_NUMBER
               AND LNACNT_ENTITY_NUM = V_GLOB_ENTITY_NUM
               AND ACNTS_ENTITY_NUM = V_GLOB_ENTITY_NUM
   ...
   PROCEDURE PROCESS_FROM_RTMPLNIA
   IS
   BEGIN
     <<READRTMPLNIA>>
      BEGIN
