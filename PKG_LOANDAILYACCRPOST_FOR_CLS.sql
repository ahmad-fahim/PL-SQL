...
-- Avinash-SONALI-17AUG2012 Bgn
  ...
  TYPE ACWISE_NONNPA_INT IS RECORD(
    M_NONNPA_ACNT_NUM             NUMBER(14),
    M_NONNPA_TOT_INT_AMT          NUMBER(18,3),
    M_CURR_CODE                VARCHAR2(3));
  TYPE T_ACWISE_NONNPA_INT IS TABLE OF ACWISE_NONNPA_INT INDEX BY PLS_INTEGER;

  M_ACWISE_NONNPA_INT T_ACWISE_NONNPA_INT;

  W_NPA_TOT_INT_AMT   NUMBER(18,3);
  W_NONNPA_TOT_INT_AMT   NUMBER(18,3);
  W_NPA_ACNT_NUM      NUMBER(14);
  W_NONNPA_ACNT_NUM      NUMBER(14);
  W_MAX_SL            NUMBER;
  NPAINDEX            NUMBER;
  W_NPA_ACT_INT_AMT   NUMBER(18,3);
  W_NONNPA_ACT_INT_AMT   NUMBER(18,3);
...
-- Avinash-SONALI-17AUG2012 End
  ...
  TYPE LOAN_PROD_CURR_GL IS RECORD(
    ...
    LNPRDAC_COMP_ACCR_GL     VARCHAR2(15),
    LNPRDAC_COMP_REAL_GL     VARCHAR2(15));
 ...
 FUNCTION FN_GET_AMOUNT_FORMAT(
  ...
  BEGIN
    SELECT L.LNPRDAC_PROD_CODE,
           ...
           LNPRDAC_COMP_ACCR_GL,
           LNPRDAC_COMP_REAL_GL BULK COLLECT
    ...
    IF MTEMPLOAN_PROD_CURR_GL.COUNT > 0 THEN
      FOR IDX IN 1 .. MTEMPLOAN_PROD_CURR_GL.COUNT LOOP
        ...
        MLOAN_PROD_CURR_GL(MTEMPLOAN_PROD_CURR_GL(IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_COMP_ACCR_GL := MTEMPLOAN_PROD_CURR_GL(IDX)
                                                                                                     .LNPRDAC_COMP_ACCR_GL;
                                                                                                     
        MLOAN_PROD_CURR_GL(MTEMPLOAN_PROD_CURR_GL(IDX).LNPRDAC_PROD_CURR_KEY).LNPRDAC_COMP_REAL_GL := MTEMPLOAN_PROD_CURR_GL(IDX)
                                                                                                     .LNPRDAC_COMP_REAL_GL;
      END LOOP;
    END IF;
  ...
  V_NPA_FLAG VARCHAR2(1);
   BEGIN
    ...
    -- Avinash-SONALI-17AUG2012 Bgn
    IF MLOANIA_INT_AMT(W_BRN_INDEX) <> 0 THEN
    V_NPA_FLAG := MLOANIA_NPA_STATUS(W_BRN_INDEX) ;
    IF V_NPA_FLAG = '1' THEN
    NULL ;
    END IF ;
      IF MLOANIA_NPA_STATUS(W_BRN_INDEX) = 1 THEN
  ...
  BEGIN
    ...
    WHILE W_INDEX_KEY IS NOT NULL LOOP
      IF M_LOANCALCNPAPOSTRECORD(W_INDEX_KEY).M_LOAN_AC_TOT_INT_ACCR_AMT <> 0 THEN
        ...
        END IF;
        IF MLOAN_PROD_CURR_GL.EXISTS(W_INDEX_KEY) THEN
          IF TRIM(MLOAN_PROD_CURR_GL(W_INDEX_KEY).LNPRDAC_COMP_ACCR_GL) IS NULL THEN
            W_ERR_MSG := 'Accounting Parameter Not Defined(ACCRUAL) - Prod Code = ' ||
                         W_PROD_CODE || '  ' || ' Curr Code = ' ||
                         W_CURR_CODE;
            RAISE E_USEREXCEP;
  ...
  BEGIN
    ...
    BEGIN
      FORALL M_INDEX IN 1 .. MLOANIA_ACNT_NUM.COUNT
        ...
        W_ERR_MSG:= MLOANIA_ACNT_NUM(SQL%BULK_EXCEPTIONS(I).ERROR_INDEX)|| ('Error: ' || I ||' Array Index: ' || SQL%BULK_EXCEPTIONS(I).ERROR_INDEX ||' Message: ' || SQLERRM(-SQL%BULK_EXCEPTIONS(I).ERROR_CODE));
  ...
  PROCEDURE UPDATE_MARKUPINTLED IS
      W_CRDB_FLG CHAR(1);
      W_CBD_TIME VARCHAR2(20);
  BEGIN
      
      
      SELECT NVL(MAX(MARKUPPROFLED_SL_NUM), 0) + 1
        INTO W_MAX_SL
        FROM MARKUPPROFLED
       WHERE MARKUPPROFLED_ENTITY_NUM = V_GLOB_ENTITY_NUM
         AND MARKUPPROFLED_ACNT_NUM = W_INTERNAL_ACNUM
         AND MARKUPPROFLED_TRAN_DATE = W_PROCESS_DATE;
      ...
      INSERT INTO MARKUPPROFLED
        (MARKUPPROFLED_ENTITY_NUM, 
        MARKUPPROFLED_ACNT_NUM, 
        MARKUPPROFLED_TRAN_DATE, 
        MARKUPPROFLED_SL_NUM, 
        MARKUPPROFLED_VALUE_DATE, 
        MARKUPPROFLED_ENTRY_TYPE, 
        MARKUPPROFLED_DB_CR_FLG, 
        MARKUPPROFLED_CURR_CODE, 
        MARKUPPROFLED_AMOUNT, 
        MARKUPPROFLED_REMARKS1, 
        MARKUPPROFLED_REMARKS2, 
        MARKUPPROFLED_REMARKS3, 
        MARKUPPROFLED_ENTD_BY, 
        MARKUPPROFLED_ENTD_ON, 
        MARKUPPROFLED_LAST_MOD_BY, 
        MARKUPPROFLED_LAST_MOD_ON, 
        MARKUPPROFLED_AUTH_BY, 
        MARKUPPROFLED_AUTH_ON)
      VALUES
        (V_GLOB_ENTITY_NUM,
         W_INTERNAL_ACNUM,
         W_PROCESS_DATE,
    ...
    EXCEPTION
      WHEN OTHERS THEN
        W_ERR_MSG := 'Error in Updating LNSUSPLED (Creation of Suspense Ledger)';
        RAISE E_USEREXCEP;
  END UPDATE_MARKUPINTLED;
  
  PROCEDURE UPDATE_MARKUPINTBAL IS
    W_DB_SUM NUMBER;
    W_CR_SUM NUMBER;
    W_CBD_TIME VARCHAR2(20);
  BEGIN
      W_CBD_TIME := W_PROCESS_DATE || ' ' ||PKG_PB_GLOBAL.FN_GET_CURR_BUS_TIME(V_GLOB_ENTITY_NUM);
      SELECT MARKUPPROFBAL_BAL
        INTO DUMMY
        FROM MARKUPPROFBAL
       WHERE MARKUPPROFBAL_ENTITY_NUM = V_GLOB_ENTITY_NUM
         AND MARKUPPROFBAL_ACNT_NUM = W_INTERNAL_ACNUM
         AND MARKUPPROFBAL_CURR_CODE = W_CURR_CODE;

      IF (W_NONNPA_ACT_INT_AMT < 0) THEN
        UPDATE MARKUPPROFBAL
           SET MARKUPPROFBAL_BAL    = MARKUPPROFBAL_BAL -
                                       ABS(W_NONNPA_ACT_INT_AMT),
               MARKUPPROFBAL_DB_SUM = MARKUPPROFBAL_DB_SUM +
                                       ABS(W_NONNPA_ACT_INT_AMT) 
         WHERE MARKUPPROFBAL_ENTITY_NUM = V_GLOB_ENTITY_NUM
         AND MARKUPPROFBAL_ACNT_NUM = W_INTERNAL_ACNUM
         AND MARKUPPROFBAL_CURR_CODE = W_CURR_CODE;
      ELSE
        IF (W_NONNPA_ACT_INT_AMT > 0) THEN
          UPDATE MARKUPPROFBAL
           SET MARKUPPROFBAL_BAL    = MARKUPPROFBAL_BAL +
                                       ABS(W_NONNPA_ACT_INT_AMT),
               MARKUPPROFBAL_CR_SUM = MARKUPPROFBAL_CR_SUM +
                                       ABS(W_NONNPA_ACT_INT_AMT) 
         WHERE MARKUPPROFBAL_ENTITY_NUM = V_GLOB_ENTITY_NUM
         AND MARKUPPROFBAL_ACNT_NUM = W_INTERNAL_ACNUM
         AND MARKUPPROFBAL_CURR_CODE = W_CURR_CODE;
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN 
      IF (W_NONNPA_ACT_INT_AMT < 0) THEN
        W_DB_SUM := ABS(W_NONNPA_ACT_INT_AMT);
        W_CR_SUM := 0;
      ELSE 
        IF (W_NONNPA_ACT_INT_AMT > 0) THEN
          W_CR_SUM := ABS(W_NONNPA_ACT_INT_AMT);
          W_DB_SUM := 0;
        END IF;
      END IF;
      ...
      <<ADD_MARKUPINTBAL>>
      BEGIN
          INSERT INTO MARKUPPROFBAL
            (MARKUPPROFBAL_ENTITY_NUM, 
            MARKUPPROFBAL_ACNT_NUM, 
            MARKUPPROFBAL_CURR_CODE, 
            MARKUPPROFBAL_BAL, 
            MARKUPPROFBAL_DB_SUM, 
            MARKUPPROFBAL_CR_SUM, 
            MARKUPPROFBAL_REMARKS1, 
            MARKUPPROFBAL_REMARKS2, 
            MARKUPPROFBAL_REMARKS3, 
            MARKUPPROFBAL_ENTD_BY, 
            MARKUPPROFBAL_ENTD_ON, 
            MARKUPPROFBAL_LAST_MOD_BY, 
            MARKUPPROFBAL_LAST_MOD_ON, 
            MARKUPPROFBAL_AUTH_BY, 
            MARKUPPROFBAL_AUTH_ON)
          VALUES
            (V_GLOB_ENTITY_NUM,
             W_INTERNAL_ACNUM,
             W_CURR_CODE,
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          RAISE E_USEREXCEP;
      END ADD_MARKUPINTBAL;
  END UPDATE_MARKUPINTBAL;
  ...
  PROCEDURE GET_ACWISE_MARKUP_PROFIT IS
  BEGIN
  W_NONNPA_TOT_INT_AMT   := 0;
  W_NONNPA_ACNT_NUM      := 0;
  W_CURR_CODE         := '';
  W_MAX_SL            := 0;
  W_NONNPA_ACT_INT_AMT   := 0;
  W_INTERNAL_ACNUM    := 0;
  DUMMY               := 0;
  NPAINDEX            := 0;
    FOR J IN 1 .. MLOANIA_ACNT_NUM.COUNT LOOP
      IF MLOANIA_NPA_STATUS(J) = 0 AND W_NONNPA_ACNT_NUM = 0 THEN
        W_NONNPA_ACNT_NUM         := MLOANIA_ACNT_NUM(J);
        W_NONNPA_TOT_INT_AMT      := NVL(MLOANIA_INT_AMT_RND(J),0) + NVL(MLOANIA_OD_INT_AMT_RND(J),0);
        W_CURR_CODE            := MLOANIA_ACNT_CURR(J);
        NPAINDEX               := 1;
      ELSIF MLOANIA_NPA_STATUS(J) = 0 AND W_NONNPA_ACNT_NUM <> MLOANIA_ACNT_NUM(J) AND W_NONNPA_ACNT_NUM <> 0 THEN
        M_ACWISE_NONNPA_INT(NPAINDEX).M_NONNPA_ACNT_NUM          :=  W_NONNPA_ACNT_NUM;
        M_ACWISE_NONNPA_INT(NPAINDEX).M_NONNPA_TOT_INT_AMT       :=  W_NONNPA_TOT_INT_AMT;
        M_ACWISE_NONNPA_INT(NPAINDEX).M_CURR_CODE             :=  W_CURR_CODE;
        NPAINDEX               := NPAINDEX + 1;
        W_NONNPA_ACNT_NUM         := MLOANIA_ACNT_NUM(J);
        W_CURR_CODE            := MLOANIA_ACNT_CURR(J);
        W_NONNPA_TOT_INT_AMT      := NVL(MLOANIA_INT_AMT_RND(J),0) + NVL(MLOANIA_OD_INT_AMT_RND(J),0);
      ELSIF MLOANIA_NPA_STATUS(J) = 0 AND W_NONNPA_ACNT_NUM = MLOANIA_ACNT_NUM(J) THEN
        W_NONNPA_TOT_INT_AMT      := W_NONNPA_TOT_INT_AMT + NVL(MLOANIA_INT_AMT_RND(J),0) + NVL(MLOANIA_OD_INT_AMT_RND(J),0);
      END IF;
    END LOOP;
        M_ACWISE_NONNPA_INT(NPAINDEX).M_NONNPA_ACNT_NUM          :=  W_NONNPA_ACNT_NUM;
        M_ACWISE_NONNPA_INT(NPAINDEX).M_NONNPA_TOT_INT_AMT       :=  W_NONNPA_TOT_INT_AMT;
    ...
    IF NPAINDEX <> 0 THEN
    FOR K IN 1 .. NPAINDEX LOOP
      IF M_ACWISE_NONNPA_INT(K).M_NONNPA_TOT_INT_AMT <> 0 THEN
        W_NONNPA_ACT_INT_AMT   := M_ACWISE_NONNPA_INT(K).M_NONNPA_TOT_INT_AMT;
        W_INTERNAL_ACNUM    := M_ACWISE_NONNPA_INT(K).M_NONNPA_ACNT_NUM;
        W_CURR_CODE         := M_ACWISE_NONNPA_INT(K).M_CURR_CODE;
        UPDATE_MARKUPINTLED;
        UPDATE_MARKUPINTBAL;
      END IF;
      W_MAX_SL := 0;
  ...
  BEGIN
    ...
    V_AC_NUM := M_RTMPLNIA_RECORD(W_ARRAY_PROCESS_SL).RTMPLNIA_ACNT_NUM ;
    <<READLOANACNT>>
    BEGIN
  ...
  -- Avinash-SONALI-17AUG2012 (end)
    W_RUN_NUMBER       := 0;--PKG_EODSOD_FLAGS.GET_RUN_NUM(PKG_ENTITY.FN_GET_ENTITY_CODE);
    W_AC_PREV_BRN_CODE := 0;
