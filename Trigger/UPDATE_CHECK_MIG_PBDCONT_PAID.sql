CREATE OR REPLACE TRIGGER UPDATE_CHECK_MIG_PBDCONT_PAID
   BEFORE UPDATE OF MIGDEP_INT_PAID_UPTO
   ON MIG_PBDCONTRACT
   FOR EACH ROW
DECLARE
   V_SQL        VARCHAR2 (100);
...
BEGIN
   ...
   UPDATE MIG_ACNTS
      SET ACNTS_INT_DBCR_UPTO = :NEW.MIGDEP_INT_PAID_UPTO
    WHERE ACNTS_ACNUM = :OLD.MIGDEP_DEP_AC_NUM;

   UPDATE MIG_DEPIA D
      SET D.DEPIA_DATE_OF_ENTRY = :NEW.MIGDEP_INT_PAID_UPTO,
          D.DEPIA_ACCR_UPTO_DATE = :NEW.MIGDEP_INT_PAID_UPTO
   ...
   THEN
      ...
      :NEW.MIGDEP_INT_ACCR_UPTO := :NEW.MIGDEP_INT_PAID_UPTO;

      :NEW.MIGDEP_INT_CALC_UPTO := :NEW.MIGDEP_INT_PAID_UPTO;
      :NEW.MIGDEP_INT_CALC_PAYABLE_UPTO := :NEW.MIGDEP_INT_PAID_UPTO;


      UPDATE MIG_ACNTS
         SET ACNTS_INT_CALC_UPTO = :NEW.MIGDEP_INT_PAID_UPTO,
             ACNTS_INT_ACCR_UPTO = :NEW.MIGDEP_INT_PAID_UPTO
       WHERE ACNTS_ACNUM = :OLD.MIGDEP_DEP_AC_NUM;
...
END UPDATE_CHECK_MIG_PBDCONT_PAID;
/
