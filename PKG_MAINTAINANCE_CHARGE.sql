CREATE OR REPLACE PACKAGE PKG_MAINTAINANCE_CHARGE IS
  ...
  -- For foreign currency 2 extra leg are added.(22-dec-2016) 
...
IS
   ...
   PROCEDURE MOVE_TO_TRANREC_DEBIT_WEFP (P_BRN_CODE    IN NUMBER,
                                         P_GL_CODE        VARCHAR2,
                                         P_CURR_CODE   IN VARCHAR2,
                                         P_TRAN_AMT    IN NUMBER,
                                         P_ASON_DATE      DATE,
   ...
   BEGIN
      ...
      PKG_AUTOPOST.PV_TRAN_REC (W_POST_ARRAY_INDEX).TRAN_BASE_CURR_CODE :=
         W_BASE_CURR;
      PKG_AUTOPOST.PV_TRAN_REC (W_POST_ARRAY_INDEX).TRAN_AMOUNT :=
         ROUND (V_BASE_CURR_CONV_RATE * P_TRAN_AMT, 3);
      PKG_AUTOPOST.PV_TRAN_REC (W_POST_ARRAY_INDEX).TRAN_BASE_CURR_EQ_AMT :=
         ROUND (V_BASE_CURR_CONV_RATE * P_TRAN_AMT, 3);
      PKG_AUTOPOST.PV_TRAN_REC (W_POST_ARRAY_INDEX).TRAN_VALUE_DATE :=
         P_ASON_DATE;
   ...
   EXCEPTION
      ...
      THEN
         W_ERROR :=
               'ERROR IN MOVE_TO_TRANREC_CREDIT_FCFP '
            || '-'
            || SUBSTR (SQLERRM, 1, 500);
         RAISE V_USER_EXCEPTION;
   END MOVE_TO_TRANREC_DEBIT_WEFP;



   PROCEDURE MOVE_TO_TRANREC_CREDIT_FCFP (P_BRN_CODE    IN NUMBER,
                                          P_GL_CODE        VARCHAR2,
                                          P_CURR_CODE   IN VARCHAR2,
                                          P_TRAN_AMT    IN NUMBER,
                                          P_ASON_DATE      DATE,
   ...
   EXCEPTION
      ...
      THEN
         W_ERROR :=
            ...
            || SUBSTR (SQLERRM, 1, 500);
         RAISE V_USER_EXCEPTION;
   END MOVE_TO_TRANREC_CREDIT_FCFP;
   ...
   IS
      V_BASE_CURR_CONV_RATE   NUMBER (18, 3) := 1;
      V_FCFP_GL               VARCHAR2 (15);
      V_WEFP_GL               VARCHAR2 (15);
      V_COUNT                 NUMBER := 0;
   BEGIN
      ...
      THEN
         ...
         SELECT COUNT (*)
           INTO V_COUNT
           FROM TFBRNPM
          WHERE     TFBRNPM_ENTITY_NUM = V_GLOB_ENTITY_NUM
                AND TFBRNPM_BRN_CODE = P_BRN_CODE
                AND TFBRNPM_CURR_CODE = P_CURR_CODE;

         IF V_COUNT > 0
         THEN
            SELECT TFBRNPM_WAGE_FC_CR
              INTO V_FCFP_GL
              FROM TFBRNPM
             WHERE     TFBRNPM_ENTITY_NUM = V_GLOB_ENTITY_NUM
                   AND TFBRNPM_BRN_CODE = P_BRN_CODE
            ...
            SELECT TFBRNPM_WAGE_FC_CR
             ...
             WHERE     TFBRNPM_ENTITY_NUM = V_GLOB_ENTITY_NUM
                   AND TFBRNPM_BRN_CODE = P_BRN_CODE
                   AND TFBRNPM_CURR_CODE = W_BASE_CURR;

            MOVE_TO_TRANREC_CREDIT_FCFP (
               P_BRN_CODE,
               V_FCFP_GL,
               P_CURR_CODE,
               P_TRAN_AMT + NVL (W_SERV_TAX_AMT, 0),
               P_ASON_DATE,
               P_NARR1,
               P_NARR2,
               P_NARR3);

            MOVE_TO_TRANREC_DEBIT_WEFP (P_BRN_CODE,
                                        V_WEFP_GL,
                                        P_CURR_CODE,
                                        P_TRAN_AMT + NVL (W_SERV_TAX_AMT, 0),
                                        P_ASON_DATE,
